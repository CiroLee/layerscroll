class Pano {
    ...
    filter(obj){
        //获取同一类型的所有元素数组
        let count = this.get(`${obj.type}.count`),
            elementsArray = new Array();
        for(let i=0;i < count; i++){
            let element = this.get(`${obj.type}[${i}]`);
            elementsArray.push(element);
        };
        for(let i in obj.key){
            elementsArray = elementsArray.filter(element=>element[i] == obj.key[i])
        }
        
        return obj.attr ? elementsArray.map(element=>element[obj.attr]):elementsArray;      
    }
    /**
     * 
